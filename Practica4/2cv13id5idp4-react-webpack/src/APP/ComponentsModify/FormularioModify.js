...
class FormularioModify extends Component {
    ...
    componentDidMount() {
        ...
        console.log(pregun);
        fetch('http://localhost:8080/2CV13ID5IDP3/API/getQuestion'+pregun)
            .then(
                response => {
    ...
    render() {
        return (
            <Form id="formify" method="post" action="http://localhost:8080/2CV13ID5IDP3/updateQuestion" encType={this.state.Radar||this.state.Puntero?"multipart/form-data":"application/x-www-form-urlencoded"}>
                <Stack gap={3}>
